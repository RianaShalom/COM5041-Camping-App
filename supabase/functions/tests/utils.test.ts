...
import { getAccessToken, getUserId, getUserIdAndToken, response } from '../_shared/utils.ts';
...
describe('Shared Utils Tests', () => {
	...
	it('getAccessToken should return a string token when invoked with a valid Authorization header', () => {
		// assert
		expect(getAccessToken(new Request('http://my-url.com', { headers: { Authorization: 'Bearer my-token' } }))).toEqual(
			'Bearer my-token',
		);
		expect(getAccessToken(new Request('http://my-url.com', { headers: { Authorization: 'Bearer null' } }))).toEqual(
			null,
		);
		expect(getAccessToken(new Request('http://my-url.com', { headers: { Authorization: 'Bearer undefined' } })))
			.toEqual(null);
	...
	it('getUserIdAndToken should return userId and token when valid request', () => {
		// arrange
		const req = new Request('http://my-url.com', {
			headers: {
				Authorization:
					'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiYWRtaW4iOnRydWUsImlhdCI6MTUxNjIzOTAyMn0.KMUFsIDTnFmyG3nMiGM6H9FNFUROf3wh7SmqJp-QV30',
			},
